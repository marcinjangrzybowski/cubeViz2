...
import Data.String.Class as SC (putStrLn,toString) 
  ...
  do args <- liftIO getArgs
     ...
     (mbFName , mbInitialSessionState) <- 
        case args of
          ...
          [] -> error "interactive mode not suported, provide name of the file!"
     ...
     -- let mbInitialSessionState = Right (SessionStateA (Env [] [] , Context []
                                                         -- [(Just "i" , Nothing)
                                                         -- ,(Just "j" , Nothing)]))
     ...
     r <- case mbInitialSessionState of
          Right initialSessionState -> do
            let (ee) = ssEnvA initialSessionState
                cub = AA.toClCubA (ssClA initialSessionState)
            -- putStrLn (show initialSessionState)
                   -- toClCub ee expr 
            let ap = AppState
                       { fileName          = Nothing --mbFName
                       , asViewport        =
                           Viewport
                             { vpAlpha = 1/5
                             , vpBeta = 0
...
initailSS (Visualisation mbNm ce@(ClExpr n _) vctx) =
    let dims = map ((\x -> (Just x , Nothing)) . fst) (filter (\(_ , (t , _)) -> t == "I") vctx)
        c = map ((\(x , (tyS , crnrs )) -> (x ,
                      case (tyS , crnrs) of
                        ("Level" , _) -> NotCType tyS
                        ("I" , _) -> NotCType tyS
                        (('T' : 'y'  : 'p' : 'e' : ' ' : _) , _) -> NotCType tyS
...
loadFile fName =
   liftIO $ do
       ...
       return $ fmap initailSS (read contents)
...
updateGL appState =
   do let drawings = drawExpr appState (asDrawMode appState)
                      ((ssEnvA . asSession) $ appState)
                      ((asCub appState))
      let tryToRen = toRenderableDI drawingInterpreter drawings
          ee = ((ssEnvA (asSession appState)))
...
drawExpr as Scaffold ee e =
   ...
   in concat (
              ...
              [                
                ...
                mkDrawCub (DefaultPTA { dptCursorAddressA = fmap fst sptCA
                          , dptShowFillA = dpShowFilling $ asDisplayPreferences as
                          , dptFillFactorA = 1.0
