...
type SmplxF = Smplx [Float]

-- type MetaColor = [Maybe Color]


type Drawing s a = [(Smplx s , a)]

type DrawingGL = Drawing [Float] Color

type Pt3D = (Float , Float , Float)


class InSpace s => RenderablePt a s where

instance RenderablePt Pt3D [Float] where

  

data Renderable rp = Point rp | Line (rp , rp) | Triangle (rp , rp , rp)


type Renderables rp = [(Renderable rp , Shade)]





toRenderable ::RenderablePt a s =>  Smplx s -> Maybe (Renderable a)
